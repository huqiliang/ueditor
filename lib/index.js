      ...
      busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
        ...
        res.ue_up = function (img_url) {
          ...
          if (config.api) {
            ...
            var lhotelCode = req.query.lhotelCode || 'L2R';
            if(!!req.session.ticket){
              yog.ralPromise('PIC_UPLOAD', {
                // method:"POST",
                data: {
                  MAX_FILE_SIZE: 200000000,
                  file: file
                },
                path: '/out/ironhide/manager/Images/UploadAndSave?lhotelGroupCode=' + req.session.lhotelGroupCode + '&ticket=' + req.session.ticket + '&lhotelCode=' + lhotelCode,
                pack: "formdata",

              }).then(function (results) {
                console.log(results)
                if (results && results.resultInfo && results.resultInfo.length > 0) {
                  res.json({
                    'url': results.resultInfo[0].url,
                    'title': req.body.pictitle,
                    'original': filename,
                    'state': 'SUCCESS'
                  });
